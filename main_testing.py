...
solar_birthday = Solar(year,month,day)
lunar_birthday = Converter.Solar2Lunar(solar_birthday)

year_zodiac = buildChart.get_year_earthly_branch(solar_birthday)
month_zodiac = buildChart.get_month_earthly_branch(solar_birthday)
day_zodiac = buildChart.get_day_earthly_branch(solar_birthday)

year_element = buildChart.get_year_heavenly_stem(solar_birthday)
month_element = buildChart.get_month_heavenly_stem(solar_birthday)
day_element = buildChart.get_day_heavenly_stem(solar_birthday)

if option == 'y':
    hour_zodiac = buildChart.get_hour_earthly_branch(hour, daylight_saving_time)
    hour_element = buildChart.get_hour_heavenly_stem(solar_birthday, hour, daylight_saving_time)
    print('                  H  D  M  Y')
